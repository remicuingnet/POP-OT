...
def main(config: ConfigParser):
    ...
    config["train_loss"]["args"]["noise_level"] = data_loader.noise_level
    ...
    if hasattr(train_loss, "u") and hasattr(train_loss, "v"):
        reparam_params = [
            {
                "params": train_loss.u,
                "lr": config["lr_u"],
                "weight_decay": config["optimizer_overparametrization"]["args"][
                    "weight_decay"
                ],
            },
            {
        ...
        optimizer_overparametrization = config.initialize(
            "optimizer_overparametrization", torch.optim, reparam_params
        )

        lr_scheduler_overparametrization = None
        # lr_scheduler_overparametrization = config.initialize(
        #     "lr_scheduler",
    ...
    if hasattr(train_loss, "set_targets"):
        train_loss.set_targets(data_loader)
...
if __name__ == "__main__":
    ...
    options = [
        ...
        CustomArgs(
            ["--consist", "--ratio_consistency"],
            type=float,
            target=("train_loss", "args", "ratio_consistency"),
        ),
        CustomArgs(
            ["--balance", "--ratio_balance"],
            type=float,
            target=("train_loss", "args", "ratio_balance"),
        ),
    ]
