...
def get_clothing(
    ...
    if train:
        ...
        train_dataset = []
        val_dataset = Clothing(
            root, cfg_trainer, test=(not train), transform=transform_val
        )
    ...
    def __init__(
        ...
        with open("%s/noisy_label_kv.txt" % self.root, "r") as f:
            ...
            for l in lines:
                ...
                img_path = os.path.join("%s/" % self.root, "images", entry[0][7:])
        ...
        if train:
            ...
            with open("%s/noisy_train_key_list.txt" % self.root, "r") as f:
                ...
                for i, l in enumerate(lines):
                    ...
                    img_path = os.path.join("%s/" % self.root, "images", l[7:])
    ...
    def __getitem__(self, index):
        ...
        if self.train:
            if self.transform_aug is not None:
                img2 = self.transform_aug(image)
            else:
                img2 = self.transform(image)
            return img, img2, target, index, 0  # target_gt

        elif self.test or self.val:
            return img, target, index, target  # target_gt
