...
def get_cifar10(
    ...
    if train:
        ...
        elif cfg_trainer["real"]:
            new_targets = cifar10n(root, cfg_trainer["real_key"])
...
def cifar10n(root, key):
    label_path = os.path.join(root, "cifar-noisy", "CIFAR-10_human.pt")
...
class CIFAR10_train(torchvision.datasets.CIFAR10):
    def __init__(
        ...
        self.train_labels = np.array(self.targets)[
            indexs
        ]  # np.array(self.train_labels)[indexs]
        self.indexs = indexs
        self.prediction = np.zeros(
            (len(self.train_data), self.num_classes, self.num_classes),
            dtype=np.float32,
        )
        self.noise_indx = []
    ...
    def instance_noise(self):
        ...
        noise_label = torch.load(
            self.root
            + "cifar-noisy/IDN_{:.1f}_C10.pt".format(self.cfg_trainer["percent"])
        )
...
class CIFAR10_val(torchvision.datasets.CIFAR10):
    ...
    def __init__(
        ...
        super(CIFAR10_val, self).__init__(
            root,
            train=train,
            transform=transform,
            target_transform=target_transform,
