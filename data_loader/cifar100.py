...
def get_cifar100(
    ...
    base_dataset = torchvision.datasets.CIFAR100(root, train=train, download=True)
    if train:
        ...
        original_labels = train_dataset.train_labels.copy()

        val_dataset = CIFAR100_val(
            root, cfg_trainer, val_idxs, train=train, transform=transform_val
        )
        if cfg_trainer["asym"]:
        ...
        elif cfg_trainer["real"] == "clean_label":
            new_targets = cifar100n(root, "clean_label", download)
            train_dataset.train_labels = np.array(new_targets)[train_dataset.indexs]
        else:
        ...
        train_dataset = []
        val_dataset = CIFAR100_val(
            root, cfg_trainer, None, train=train, transform=transform_val
        )
        print(f"Test: {len(val_dataset)}")

    if train_dataset != []:
        noise_level = np.mean(original_labels != train_dataset.train_labels)
        print(f"original_label shape: {original_labels.shape}")
    else:
        noise_level = np.NaN
...
def cifar100n(root, key, download=True):
    label_path = os.path.join(root, "cifar-noisy", "CIFAR-100_human.pt")
    if not os.path.exists(label_path):
        raise FileNotFoundError(
            f"Labels not found. Please set download=True. Path: {label_path}"
        )
...
class CIFAR100_train(torchvision.datasets.CIFAR100):
    def __init__(
        ...
        super(CIFAR100_train, self).__init__(
            root,
            train=train,
            transform=transform,
        ...
        self.prediction = np.zeros(
            (len(self.train_data), self.num_classes, self.num_classes), dtype=np.float32
        )
        self.noise_indx = []
    ...
    def symmetric_noise(self):
        ...
        for i, idx in enumerate(indices):
            if i < self.cfg_trainer["percent"] * len(self.train_data):
                ...
                self.train_labels[idx] = np.random.randint(
    ...
    def asymmetric_noise(self, asym=False, random_shuffle=False):
        ...
        if n > 0.0:
            ...
            y_train_noisy = self.multiclass_noisify(
                self.train_labels, P=P, random_state=0
            )
    ...
    def instance_noise(self):
        ...
        noise_label = torch.load(
            self.root
            + "cifar-noisy/IDN_{:.1f}_C100.pt".format(self.cfg_trainer["percent"])
        )
...
class CIFAR100_val(torchvision.datasets.CIFAR100):
    ...
    def __init__(
        ...
        super(CIFAR100_val, self).__init__(
            root,
            train=train,
            transform=transform,
            target_transform=target_transform,
